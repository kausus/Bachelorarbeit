...
\todo{folgende begriffe müssen definiert, verwendet und verstanden sein: simplex, seite, eckmenge, dimension, simplizialkomplex, n-skelett, geometrischer simplizialkomplex, geometrische realisierung, schwache topologie, triangulierbar, simpliziale abbildung, baryzentrische koordinaten,}
...
\begin{Bsp}
	\begin{enumerate}[1)]
		...
		\todo{beispiel der zweifache Überlagerung der $S^1$}
...
%    Rand $\partial\sigma$ auf die $\S^{n-1}$ abbildet.
%  \end{enumerate}
%  \begin{proof}
%    \begin{enumerate}[(a):]
%      % a)
...
%        $f : \R^{n+1} \setminus \sset{0} \rightarrow \S^{n}, f(x) =
%        \frac{x}{\nn[x]}$.
...
%      $f_{| \partial\sigma} : \partial\sigma \rightarrow \S^{n-1}$
%      bijektiv und stetig. Da $\partial\sigma$ und $\S^{n-1}$ kompakt
%      und hausdorffsch sind, ist die Abbildung auch ein
%      Homöomorphismus.
%      
%      Setze nun als Umkehrabbildung $g : \S^{n-1} \rightarrow \partial\sigma$ 
