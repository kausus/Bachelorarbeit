    ...
    \begin{smallproof}(Nachweis das dies ein Hauptfaserbündel ist) Die
      ...
      dies ein Hauptfaserbündel mit Strukturgruppe $G$.
		...
		\begin{smallproof}
                  ...
                  die Gruppenwirkung innerhalb des Tangentialraums
                  transformiert.
			
                  Sei nun ein Bündelatlas
                  $ \sset{(U_i,\varphi_i)_{i\in \Lambda}} $ von $ \TT M $
                  gegeben. Betrachte nun die Abbildungen
                  \DefMap{\pi^{-1}(U_i)}{U_i\times
                    \GL_n}{((v_j),p)}{(p,\varphi_i(v_j)).} Diese liefern
...
\begin{Satz}\label{anzahlspin}
	...
	\begin{proof}
		...
		\DefMap{P^1_{\spin_2}{M}}{P^2_{\spin_2}{M}}{ {[p,a]}}{{[\eta(p),a]}}
		...
		mit der Bijektion 
		\DefMap{ \tilde{M} \times_\chi \Z_2 }{ \tilde{M} \times \Z_2 }{ \left[ a {,} p \right]  }{ (p,a) }
%		\DefMap{\tilde{M}\times_\chi \Z_2}{\tilde{M}\times \Z_2}{\left[p,a]\right}{(p,a)} 
		...
		\begin{center}
		\begin{tikzcd}
			U_i \cap U_j \arrow[r]  \arrow[rd]  \arrow[rdd] & \Z_2 \arrow[d] \\
			& \spin_2 \arrow[d] \\
			& \so_2 
		\end{tikzcd}
		\end{center}
...
%  	> Bei den 4 verschiedenen Spinstrukturen des 2-Torus
%  	> hatten Sie mir einen simplen Grund genannt warum
%  	> diese nicht isomorph als Spinstrukturen sein können.
%  	> Man wähle einen flachen Zusammenhang 
%  	Achtung nicht jeder flache Zusammenhang auf TT^2 ist der
%  	Levi-Civita-Zusammenhang einer flachen Metrik auf T^2.
%  	> und betrachte
%  	> dann Kurven entlang den Erzeugen der Fundamenalgruppe.
%  	> Nun müsste man den parallel Transport entlang dieser Kurven
%  	> betrachten. Doch scheint es mir relativ aufwendig diese
%  	> zu berechnen. 
%  	Nein, das ist ganz einfach zumindest, wenn man T^2=R^2\Z^2 schreibt und
%  	auf R^2 die Standardmetrik nimmt. (Man kann auch Z^2 durch jedes Gitter
%  	setzen). Dann ist der Standard-Rahmen parallel.
  \end{proof}
...
%	    Sei die Spinordarstellung $ \abb{\rho}{\spin_2}{\End(\c)} $ gegeben, dann betrachte das 
%	    $ \spin_2 $-Hauptfaserbündel $ \so_2 \times_\rho S $. Dann erhält man
%	    hieraus das gewünschte Hauptfaserbündel.
%	    
	    \todo{andere richtung, entweder mu abbildung nutzen um Pspin zu definieren, oder aus dem ass bündel die spinstr zurückgewinnen}
